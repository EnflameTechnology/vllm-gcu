...
void moe_align_block_size_gcu(at::Tensor topk_ids, int64_t num_experts,
...
void moe_align_block_size(at::Tensor topk_ids, int64_t num_experts,
  ...
  at::Tensor topk_ids_cpu, sorted_token_ids_cpu, experts_ids_cpu,
      num_tokens_post_pad_cpu;

  if (fallback_ops.has_value()) {
    if (fallback_ops->find("moe_align_block_size") != std::string::npos ||
        (*fallback_ops) == "all") {
      is_fallback = true;
      ...
      int result = vllmMoeAlignBlockSize(sorted_token_ids_cpu, experts_ids_cpu,
  ...
  moe_align_block_size_gcu(topk_ids, num_experts, block_size, sorted_token_ids,
