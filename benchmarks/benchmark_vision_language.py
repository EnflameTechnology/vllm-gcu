...
import numpy as np
from vllm_utils.vision_language.interface import get_vlm_input_obj,get_demo_mm_request, \
    get_dummy_mm_request, get_dataset_mm_request, dataset_evaluate
    ...
    **kwargs
    ...
    for _ in range(num_iters):
        ...
        for j in tqdm(range(math.ceil(num_request / batch_size))):
            tmp_req = requests.mm_data_list[j *
    ...
    if not engine_args.disable_log_stats:
        time_per_output_token = (
            llm.llm_engine.stat_loggers['prometheus'].metrics.histogram_time_per_output_token
        )
...
def vllm_perf(
    ...
    for output in outputs:
        if output.metrics.first_token_time is None:
            ...
            prefill_times.append(output.metrics.first_token_time -
    ...
    if decode_elapsed_time is None:
        print(
            "decode elapsed time is not recorded as set `--disable_log_stats`"
        )
...
def main(args: argparse.Namespace):
    ...
    ignore_eos = False
    if args.demo:
        requests = get_demo_mm_request(vlm_input_obj,
                                     args.prompt,
                                     args.input_vision_file,
                                     args.max_output_len,
                                     **input_kwargs)
    elif args.acc:
        requests = get_dataset_mm_request(vlm_input_obj,
                                        args.dataset_name,
                                        args.dataset_file,
                                        args.max_output_len,
                                        args.num_prompts,
        ...
        ignore_eos = True
        requests = get_dummy_mm_request(vlm_input_obj,
                                      args.batch_size,
                                      args.input_len,
    ...
    if args.backend == "vllm":
        ...
        if args.demo:
            for request, output in zip(requests.mm_data_list, outputs):
                print(f"Prompt: {request.origin_prompt}, "
                    f"Generated text: {output.outputs[0].text}")

        if args.perf:
            perf_info = vllm_perf(
                requests, outputs, elapsed_time, decode_elapsed_time)
    ...
    total_num_tokens = sum(len(item.prompt_token_ids) + 
                           len(item.outputs[0].token_ids) for item in outputs)
    print(f"Throughput: {len(requests.mm_data_list) / elapsed_time:.2f} requests/s, "
        f"{total_num_tokens / elapsed_time:.2f} tokens/s")


if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Benchmark vision language model.")
    parser.add_argument(
        "--backend", type=str, choices=["vllm"], default="vllm")
    ...
    parser.add_argument(
        "--demo", action="store_true", help="offline inference example")
    ...
    parser.add_argument("--input-len",type=int,
        default=None,help="Input prompt length for each request")
    ...
    parser.add_argument(
        ...
        choices=VLM_DATASETS.keys() ,help="Name of the dataset.")
    parser.add_argument(
    ...
    parser.add_argument("--mm-per-prompt",type=int,default=1,
