...
# Default: True (first chunk comes from prefill)
VLLM_GCU_PD_FIRST_TOKEN_FROM_P = os.environ.get("VLLM_GCU_PD_FIRST_TOKEN_FROM_P", "true").lower() in ("1", "true", "yes")
...
VLLM_GCU_ROUTER_ENABLE_DPLB = os.environ.get("VLLM_GCU_ROUTER_ENABLE_DPLB", "false").lower() in ("1", "true", "yes")

# Environment variable to control client read timeout; default 1200 seconds.
# Force to int in case a float string is provided.
VLLM_GCU_PROXY_CLIENT_READ_TIMEOUT = int(float(os.environ.get("VLLM_GCU_PROXY_CLIENT_READ_TIMEOUT", "3600")))
...
async def lifespan(app: FastAPI):
    ...
    timeout = httpx.Timeout(
        connect=600,
        read=VLLM_GCU_PROXY_CLIENT_READ_TIMEOUT,
        write=1200,
        pool=1200
    )
    ...
    # Create load balancer for prefill clients (optional, controlled by env var)
    app.state.prefill_load_balancer = None
    if VLLM_GCU_ROUTER_ENABLE_DPLB:
        from token_load_balancer import AsyncTokenLoadBalancer
        model = os.getenv("DPLB_MODEL_PATH", "")
        if model and os.path.exists(model):
            app.state.prefill_load_balancer = AsyncTokenLoadBalancer(
                model=model,
                prefill_clients=app.state.prefill_clients)
            print(f"DPLB (Token load balancer) ENABLED with model: {model}")
        else:
            print(f"WARNING: DPLB enabled but model path {model} does not exist, falling back to round-robin")
    else:
        print("DPLB (Token load balancer) DISABLED, using round-robin")
...
async def _handle_completions(api: str, request: Request):
    try:
        ...
        # Get the next prefill client (use load balancer if available, otherwise round-robin)
        logger.info("Send to prefiller, req id: %s", request_id)
        if request.app.state.prefill_load_balancer is not None:
            prefill_client_info = await request.app.state.prefill_load_balancer.analyze(
                request_id, req_data)
        else:
        ...
        # Release load balancer state if using load balancer
        if request.app.state.prefill_load_balancer is not None:
            await request.app.state.prefill_load_balancer.release(request_id)
...
if __name__ == "__main__":
    ...
    print(f"VLLM_GCU_ROUTER_ENABLE_DPLB: {VLLM_GCU_ROUTER_ENABLE_DPLB}")
