...
def schedule(self) -> SchedulerOutput:
    ...
    while req_index < len(self.running) and token_budget > 0:
        # for convenience, make sure every decoding requests has enough token budgets for 1 + spec_tokens tokens
        # so that we don't have to do padding
    ...
    if not preempted_reqs:
        while self.waiting and token_budget > 0:
            # for PD disaggregation, make sure first token + spec has enough token budget
            if gcu_envs.VLLM_GCU_ENABLE_DEEPSEEK_MTP_FUSION and token_budget <= self.num_lookahead_tokens:
                break
...
patch.object(sched.Scheduler, "schedule", schedule).start()
