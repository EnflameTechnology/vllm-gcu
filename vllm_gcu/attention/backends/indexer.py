...
class GCUDeepseekV32IndexerMetadataBuilder(DeepseekV32IndexerMetadataBuilder):
    ...
    def maybe_chunk_sequence_parallel(
        self,
        chunk_in: DeepseekV32IndexerPrefillChunkMetadata,
        cu_seq_q: torch.Tensor | None = None,
        num_decode_tokens: int | None = None,
    ):
        assert isinstance(chunk_in, (DeepseekV32IndexerPrefillChunkMetadata, DeepSeekV32IndexerDecodeMetadata))

        if not self.indexer_use_sp_q:
            return chunk_in

        def get_sp_chunks(num: int):
            tp_group = get_tp_group()
            sp_scatters = scatter(num, tp_group.world_size)
            sp_chunk_start = sum(sp_scatters[: tp_group.rank_in_group])
            sp_chunk_end = sum(sp_scatters[: tp_group.rank_in_group + 1])
            return (sp_chunk_start, sp_chunk_end)

        if isinstance(chunk_in, DeepseekV32IndexerPrefillChunkMetadata):
            assert cu_seq_q is not None
            sum_seq_q = chunk_in.token_end - chunk_in.token_start
            sp_chunk_start, sp_chunk_end = get_sp_chunks(sum_seq_q)
            ...
            assert num_decode_tokens is not None
            num_decodes = num_decode_tokens if num_decode_tokens == 0 else chunk_in.seq_lens.size(0)
            sp_chunk_start, sp_chunk_end = get_sp_chunks(num_decodes)

            block_table = chunk_in.block_table[sp_chunk_start: sp_chunk_end]
            seq_lens = chunk_in.seq_lens[sp_chunk_start: sp_chunk_end]
            decode_lens = chunk_in.decode_lens[sp_chunk_start: sp_chunk_end]
            self.scheduler_metadata_buffer[:] = get_paged_mqa_logits_metadata(
                seq_lens, self.kv_cache_spec.block_size, self.num_sms
            )
            chunk_out = DeepSeekV32IndexerDecodeMetadata(
                block_table=block_table,
                seq_lens=seq_lens,
                decode_lens=decode_lens,
                requires_padding=chunk_in.requires_padding,
                schedule_metadata=self.scheduler_metadata_buffer,
            )
            chunk_out.gathered_slice = slice(sp_chunk_start, sp_chunk_end)
        return chunk_out
    ...
    def build(self,
        ...
        with split_ctx, mqa_metadata_ctx:
            ...
            if r.prefill:
                r.prefill.chunks = [
                    self.maybe_chunk_sequence_parallel(
                        chunk, cu_seq_q=common_attn_metadata.query_start_loc_cpu
                    )
                    for chunk in r.prefill.chunks
                ]
            if r.decode:
                r.decode = self.maybe_chunk_sequence_parallel(r.decode, num_decode_tokens=r.num_decode_tokens)
