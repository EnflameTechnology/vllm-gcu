...
from vllm_gcu.attention.ops.paged_attn import PagedAttention, PagedAttentionMetadata
...
@dataclass
class GCUXFormersMetadata(AttentionMetadata, PagedAttentionMetadata):
    """Metadata for XFormersbackend.

    NOTE: Any python object stored here is not updated when it is
    gcu-graph replayed. If you have values that need to be changed
    dynamically, it should be stored in tensor. The tensor has to be
    updated from `GCUGraphRunner.forward` API.
    """
    ...
    # Whether or not if gcu graph is enabled.
    # Gcu-graph is currently enabled for decoding only.
    use_cuda_graph: bool
    ...
    _cached_prefill_metadata: Optional["GCUXFormersMetadata"] = None
    _cached_decode_metadata: Optional["GCUXFormersMetadata"] = None
    ...
    def advance_step(
        ...
        """
        # When using gcugraph, the num_seqs is padded to the next captured
...
class GCUXFormersImpl(AttentionImpl[GCUXFormersMetadata]):
    ...
    Generation tokens can contain padding when gcu-graph is used.
    ...
    Currently, gcu graph is disabled for chunked prefill, meaning there's no
        ...
        if attn_bias is None:
            if self.alibi_slopes is None:
                ...
                # attn_metadata.encoder_seq_lens
                    ...
                    attn_bias = BlockDiagonalMask.from_seqlens(
                        attn_metadata.encoder_seq_lens, device="gcu"
                    )
