...
from vllm.model_executor.models.interfaces import MultiModalEmbeddings, SupportsMRoPE, SupportsMultiModal
...
class PaddleOCRVLDummyInputsBuilder(BaseDummyInputsBuilder[PaddleOCRVLProcessingInfo]):
    ...
    def get_dummy_mm_data(
        ...
        mm_options: Mapping[str, int] | None = None,
    ) -> MultiModalDataDict:
        num_images = mm_counts.get("image", 0)
...
class PaddleOCRVLForConditionalGeneration(nn.Module, SupportsMultiModal, SupportsMRoPE):
    ...
    def _parse_and_validate_image_input(
        ...
        ## 当bs>1时，pixel_values可能为list of tensors，因为图片分辨率不同
        if isinstance(pixel_values, np.ndarray):
            pixel_values = torch.from_numpy(pixel_values).float()
        elif not isinstance(pixel_values, torch.Tensor):
            tensors = [torch.as_tensor(p) for p in pixel_values]
            if all(t.shape == tensors[0].shape for t in tensors):
                pixel_values = torch.stack(tensors)
            else:
                pixel_values = tensors  # 保持 list
        ## 当bs>1时，image_grid_thw可能为list of tensors
        if isinstance(image_grid_thw, np.ndarray):
            ...
            tensors = [torch.as_tensor(p) for p in image_grid_thw]
            if all(t.shape == tensors[0].shape for t in tensors):
                image_grid_thw = torch.stack(tensors)
            else:
                image_grid_thw = tensors
        if isinstance(pixel_values, torch.Tensor):
            if len(pixel_values.shape) == 6 and pixel_values.shape[1] == 1:
                pixel_values = pixel_values.squeeze(1)
        if isinstance(image_grid_thw, torch.Tensor):
            if len(image_grid_thw.shape) == 3 and image_grid_thw.shape[1] == 1:
                image_grid_thw = image_grid_thw.squeeze(1)

        if pixel_values is None:
            return None

        # pixel_values 为 list 时各元素 shape 可能不同，TensorSchema.validate() 会要求
        # 一致并报错，故跳过校验；下游 _process_image_input 已用 zip(pixel_values, ...) 支持 list
        validate = isinstance(pixel_values, torch.Tensor)
