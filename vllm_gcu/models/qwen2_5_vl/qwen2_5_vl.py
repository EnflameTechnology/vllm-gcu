...
class Qwen2_5_VisionTransformer(nn.Module):
    ...
    def get_rotary_pos_emb(self, rotary_pos_emb_tuple):
        rotary_pos_emb = torch.cat(rotary_pos_emb_tuple)
        rotary_pos_emb = rotary_pos_emb.to(device=self.device,
                                           non_blocking=True)
        cos = rotary_pos_emb.cos()
        sin = rotary_pos_emb.sin()
        cos_sin = torch.concat([cos, sin], -1).to(self.dtype)
        rotary_dim = cos.shape[0]
        return cos_sin, rotary_dim
    ...
    def forward(
        ...
        rotary_pos_emb, rotary_dim = self.get_rotary_pos_emb(tuple(rotary_pos_emb))
...
class Qwen2_5_VLForConditionalGeneration(nn.Module, SupportsMultiModal,
    ...
    def _maybe_ignore_quant_config(self, config: Optional[QuantizationConfig]):
        ...
        if isinstance(config, (GPTQConfig, GPTQMarlinConfig, Fp8GCUConfig)):
            return None
        return config
