...
from .mm_encoder_attention import MMEncoderAttention
...
from vllm.model_executor.models.utils import WeightsMapper, init_vllm_registered_model, maybe_prefix
...
def apply_rotary_emb(freqs, t, start_index=0, scale=1.0, seq_dim=-2):
    ...
    assert (
        rot_dim <= t.shape[-1]
    ), "feature dimension {} is not of sufficient size to rotate in all the " "positions {}".format(
        t.shape[-1], rot_dim
    )

    indices = [start_index, end_index-start_index, t.shape[-1]-end_index]
    t_left, t, t_right = torch.split(t, indices, dim=-1)
