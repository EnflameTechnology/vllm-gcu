...
class GCUModelRunner(LoRAModelRunnerMixin):
    ...
    def __init__(
        ...
        self.device_properties = torch.gcu.get_device_properties(self.device)
    ...
    def _update_states(self, scheduler_output: "SchedulerOutput") -> None:
        ...
        for req_data in scheduler_output.scheduled_cached_reqs:
            ...
            num_new_tokens = (
                num_computed_tokens + len(req_data.new_token_ids) - req_state.num_tokens
            )
            if num_new_tokens == 1:
    ...
    def execute_model(
        ...
        if (
            ...
            has_prefill = torch.tensor(
                1 if attn_metadata.num_prefills > 0 else 0, dtype=torch.int32
            )
            ...
            # TODO: force eager for moe or split graph
        ...
        if max_gen_len == 1:
            ...
            valid_sampled_token_ids = [
                seq.tolist() for seq in sampled_token_ids[valid_mask].split(gen_lens)
            ]
    ...
    def _dummy_sampler_run(
        ...
        dummy_tensors = lambda v: torch.full(  # noqa: E731
        ...
        try:
            ...
            if "out of memory" in str(e):
                raise RuntimeError(
                    "GCU out of memory occurred when warming up sampler with "
    ...
    def capture_model(self) -> None:
        if not self.use_cuda_graph:
            logger.warning(
                "Skipping CUDA graph capture. Please add " "-O %s to use CUDA graphs.",
        ...
        with graph_capture(device=self.device):
            reversed_cudagraph_batch_sizes = reversed(self.cudagraph_batch_sizes)
            cudagraph_capture_sizes = (
                tqdm(reversed_cudagraph_batch_sizes, desc="Capturing TOPS graph shapes")
                if get_tensor_model_parallel_rank() == 0
                else reversed_cudagraph_batch_sizes
            )
            ...
            if (
                ...
                additional_config.update({"all_dp_in_decode": True})
            for num_tokens in cudagraph_capture_sizes:
                for _ in range(
        ...
        graph_size = start_free_gpu_memory - end_free_gpu_memory
