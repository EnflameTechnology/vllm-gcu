...
class GCUModelRunner(GPUModelRunner):
    ...
    def _prepare_inputs(
        ...
        if self.uses_mrope:
            # Only relevant for models using M-RoPE (e.g, Qwen2-VL)
            # This is numerically equivalent to the following:
            # self.mrope_positions.gpu[:, :total_num_scheduled_tokens].copy_(
            #     self.mrope_positions.cpu[:, :total_num_scheduled_tokens],
            #     non_blocking=True)
            # However, copy_h2d from pinned non-contiguous CPU tensor creates
            # a contiguous yet UNPINNED CPU intermediate buffer, which causes
            # an unwanted stream sync. 
