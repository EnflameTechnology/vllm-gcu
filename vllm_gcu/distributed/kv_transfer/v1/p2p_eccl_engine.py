...
from vllm_gcu.distributed.pyeccl_wrapper import ECCLLibrary, buffer_type, topsStream_t, ecclDataTypeEnum, ecclComm_t
...
@contextmanager
def set_p2p_eccl_context(num_channels: str):
    ...
    logger.info("set_p2p_eccl_context, original_values: %s", original_values)

    try:
        os.environ['ECCL_MAX_NCHANNELS'] = num_channels
        os.environ['ECCL_MIN_NCHANNELS'] = num_channels
        os.environ['ECCL_CUMEM_ENABLE'] = '1'
        yield
    finally:
        for var in env_vars:
...
class P2pEcclEngine:
    ...
    def __init__(self,
        ...
        self.send_stream = torch.gcu.Stream()
        self.recv_stream = torch.gcu.Stream()
        ...
        self.comms: dict[str, Any] = {}  # remote_address: (ecclComm_t, rank)

        self.buffer_size = 0
        self.buffer_size_threshold = float(self.config.kv_buffer_size)

        self.eccl_num_channels = self.config.get_from_extra_config(
            "eccl_num_channels", "8")
        ...
        logger.info(
            "💯P2pEcclEngine init, rank:%d, local_rank:%d, http_address:%s, "
            "zmq_address:%s, proxy_address:%s, send_type:%s, buffer_size_"
            "threshold:%.2f, eccl_num_channels:%s", self.rank, self.local_rank,
            self.http_address, self.zmq_address, self.proxy_address,
            self.send_type, self.buffer_size_threshold, self.eccl_num_channels)
    ...
    def _create_connect(self, remote_address: typing.Optional[str] = None):
        ...
        if remote_address not in self.socks:
            ...
            unique_id = self.eccl.ecclGetUniqueId()
            data = {"cmd": "NEW", "unique_id": bytes(unique_id.internal)}
            sock.send(msgpack.dumps(data))

            with torch.gcu.device(self.device):
                rank = 0
                with set_p2p_eccl_context(self.eccl_num_channels):
                    comm: ecclComm_t = self.eccl.ecclCommInitRank(
                        2, unique_id, rank)
                self.comms[remote_address] = (comm, rank)
                logger.info("🤝ecclCommInitRank Success, %s👉%s, MyRank: %s",
    ...
    def _listen_for_requests(self):
        while True:
            ...
            if self.router_socket in socks:
                ...
                data = msgpack.loads(message)
                if data["cmd"] == "NEW":
                    unique_id = self.eccl.unique_id_from_bytes(
                    ...
                    with torch.gcu.device(self.device):
                        ...
                        logger.info(
                            "🤝ecclCommInitRank Success, %s👈%s, MyRank:%s",
                            self.zmq_address, remote_address.decode(), rank)
                elif data["cmd"] == "PUT":
                    tensor_id = data["tensor_id"]
                    try:
                        with torch.gcu.stream(self.recv_stream):
                            tensor = torch.empty(data["shape"],
                                                 dtype=getattr(
                                                     torch, data["dtype"]),
                                                 device=self.device)
                    ...
                    except torch.gcu.OutOfMemoryError:
                        self.router_socket.send_multipart(
                            [remote_address, b"1"])
                        tensor = None
                        logger.warning(
    ...
    def _send(self, comm, tensor: torch.Tensor, dst: int, stream=None):
        ...
        with torch.gcu.stream(stream):
            self.eccl.ecclSend(buffer_type(tensor.data_ptr()), tensor.numel(),
                               ecclDataTypeEnum.from_torch(tensor.dtype), dst,
                               comm, topsStream_t(stream.gcu_stream))
        stream.synchronize()
    ...
    def _recv(self, comm, tensor: torch.Tensor, src: int, stream=None):
        ...
        with torch.gcu.stream(stream):
            self.eccl.ecclRecv(buffer_type(tensor.data_ptr()), tensor.numel(),
                               ecclDataTypeEnum.from_torch(tensor.dtype), src,
                               comm, topsStream_t(stream.gcu_stream))
        stream.synchronize()
