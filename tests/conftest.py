    ...
    def __init__(
        ...
        *,
        ...
        if is_sentence_transformer:
            ...
            self.model = CrossEncoder(model_name, device="cpu", trust_remote_code=True)
            self.model.model = self.wrap_device(self.model.model).to(dtype=torch_dtype)
        else:
        ...
        for i, (encoder_prompt, decoder_prompt) in enumerate(
            ...
            if decoder_prompt is None:
                ...
                decoder_input_ids = self.wrap_device(
                    self.tokenizer(decoder_prompt, return_tensors="pt").input_ids,
                    device=self.model.device.type,
                )
...
class VllmRunner:
    ...
    def generate_beam_search(
        ...
        outputs = self.model.beam_search(
            prompts, BeamSearchParams(beam_width=beam_width, max_tokens=max_tokens)
        )
        returned_outputs = []
