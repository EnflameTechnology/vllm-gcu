...
from vllm_gcu.attention.backends.flashmla_sparse import (
    FlashMLASparseBackend, FlashMLASparseMetadata, FlashMLASparseImpl)
...
def _quantize_dequantize_fp8_ds_mla(
        ...
        scale: torch.Tensor, kv_cache_dtype='fp8_ds_mla') -> tuple[torch.Tensor, torch.Tensor]:
    ...
    for token_idx in range(num_tokens):
        ...
        if kv_cache_dtype == 'fp8_ds_mla':
            ...
            latent = cache_slice[:kv_lora_rank].to(kv_c.dtype)
            rope_vals = cache_slice[kv_lora_rank:].to(k_pe.dtype)
...
@pytest.mark.skip(reason="skip test_sparse_decode_metadata_filters_prefill_indices")
...
# @pytest.mark.skip(reason="skip test_sparse_backend_decode_correctness")
@pytest.mark.parametrize("batch_name", list(SPARSE_BACKEND_BATCH_SPECS.keys()))
@pytest.mark.parametrize("kv_cache_dtype", ["fp8_ds_mla", "auto", "fp8"])
def test_sparse_backend_decode_correctness(dist_init, batch_name,
                                           kv_cache_dtype):
    if kv_cache_dtype == 'auto':
        pytest.skip("GCU not support auto yet")
    if batch_name in {"mixed_small", "mixed_medium", "small_prefill", "medium_prefill", "single_prefill"}:
        pytest.skip("GCU not support small batch yet")
    ...
    for i in range(batch_spec.batch_size):
        ...
        ql_nope = torch.einsum("qnh,lnh->qnl", q_nope.cpu(), W_UK.cpu()).gcu()
        ...
        sdpa_out = torch.nn.functional.scaled_dot_product_attention(
            q_sdpa_in.cpu(), k_sdpa_in.cpu(), v_sdpa_in.cpu(), attn_mask=attn_mask.cpu(), scale=scale)
        sdpa_out = sdpa_out.transpose(1, 2).squeeze(0).gcu()

        sdpa_out = torch.einsum("qnl,lnv->qnv", sdpa_out.cpu(), W_UV.cpu()).gcu()
    ...
    kv_cache = create_and_prepopulate_kv_cache(
        ...
        block_size=vllm_config.cache_config.block_size,
        head_size=head_size,
        dtype=dtype if kv_cache_dtype != 'fp8' else torch.float8_e4m3fn,
        device=device,
    ...
    print(f"---debug--- backend_output: {backend_output}")
