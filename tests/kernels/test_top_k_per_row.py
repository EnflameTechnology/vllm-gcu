...
def create_random_logits(
    ...
    if data_generation == "random":
        logits = torch.randn(
            row_starts.shape[0], max(row_ends), dtype=dtype, device="gcu"
        )
...
def create_row_boundaries(
    ...
    row_starts = torch.zeros(seq_len, dtype=torch.int32, device="gcu")
    row_ends = torch.arange(1, seq_len + 1, device="gcu", dtype=torch.int32)
...
def compare_top_k_results(
    ...
    """
    Compare results from GCU top_k_per_row with torch.topk.
...
def _run_top_k_per_row_decode_test(
    ...
    """
    torch.set_default_device("gcu:0")
    # Create test data
    num_rows = batch_size * next_n
    seq_lens = torch.randint(vocab_size, (batch_size,), dtype=torch.int32, device="gcu")
    ...
    ), "GCU top_k_per_row_decode results don't match torch.topk"
...
def test_top_k_per_row_decode(
    top_k: int, batch_size: int, next_n: int, data_generation: str, threshold: int
) -> None:
    """
    ...
    _run_top_k_per_row_decode_test(
        top_k, batch_size, next_n, vocab_size, data_generation, threshold
    )
...
@pytest.mark.parametrize("threshold", THRESHOLD)
@torch.inference_mode()
def test_top_k_per_row(num_rows: int, top_k: int, threshold: int) -> None:
    """
    Test top_k_per_row.
    """
