...
def _run_convert_req_index_to_global_index_kernel(
    req_id: torch.Tensor,
    block_table: torch.Tensor,
    token_indices: torch.Tensor,
    ...
    """Run convert_req_index_to_global_index kernel and return output."""
    out = torch.empty_like(token_indices)
    torch.ops._C_cache_ops.convert_req_index_to_global_index(
        out,
        req_id,
...
@pytest.mark.parametrize("case", [
    ...
    # block_size, block_n
    (128, 16, 8, 64, 64, 64),
    (256, 32, 16, 128, 64, 128),
])
@pytest.mark.parametrize("add_invalid", [False, True])
@torch.inference_mode()
def test_convert_req_index_to_global_index_native_override(case, add_invalid):
    """
    Test convert_req_index_to_global_index by comparing kernel output with native Python override.
    """
    ...
    # Run original kernel
    out_kernel = _run_convert_req_index_to_global_index_kernel(
        req_id,
        block_table,
        token_indices,
        prefill_workspace_request_ids,
    ...
    register_native_overrides({"fallback_ops": ["convert_req_index_to_global_index"]})
    out_override = _run_convert_req_index_to_global_index_kernel(
        req_id,
        block_table,
        token_indices,
        prefill_workspace_request_ids,
    ...
    restore_native_overrides("_C_cache_ops")

    # Compare results
    torch.testing.assert_close(
        out_kernel, out_override, rtol=1e-4, atol=1e-4,
        msg="Kernel output doesn't match native override for convert_req_index_to_global_index"
    )
