...
def _run_kernel(input_tensor: torch.Tensor) -> torch.Tensor:
    ...
    torch.ops._C.swigluoai_and_mul(out, input_tensor)
    return out

@pytest.mark.parametrize("dtype", [torch.float16, torch.float32])
@torch.inference_mode()
def test_swigluoai_and_mul_compare_via_override(dtype):
    ...
    # 覆盖实现为 python 参考版本
    register_native_overrides({"fallback_ops": ["swigluoai_and_mul"]})
